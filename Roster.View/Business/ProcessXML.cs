...
namespace FlightSchedules.View.Business
{
    public static class ProcessXML
    {
            ...
            bool isMappingZone = true)
        {
            ...
            souceNotAssign = source.Where(p => works.Contains(p.ShortName) && !idAssigned.Contains(p.Id)).ToList();
            List<ScheduleInfo> planWorkAcceptMinWork = DoWorkServicePlan(souceNotAssign, workSchedules1, shortCode, works, dateWork, numbeLoop, jobConfigs, (identity + 100), false, 10, true);             
            ...
            ////Lấy công việc của  Lái Xe băng tải
            works.Add("BTA");
            idAssigned = resultChangeShiftTow.Select(p => p.Id).Distinct().ToList();
            souceNotAssign = source.Where(p => works.Contains(p.ShortName) && !idAssigned.Contains(p.Id)).ToList();
            return ProcessCommon.BonusWorkForUser(souceNotAssign, resultChangeShiftTow, false);
        }       
            ...
            bool isAcceptionMinWork = false)
        {
            ...
            for (int i = 0; i < numbeLoop; i++)
            {
                ...
                var worksOffUserFake = ProcessCommon.ProcessWorkForUserFake(sourcePlanWorkByShortName, shortCode, identityUserFake, i, isAcceptionMinWork, i, isMappingZone);
